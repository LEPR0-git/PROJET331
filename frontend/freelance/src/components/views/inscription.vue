  ...
  <div class="auth-container">
    <div class="auth-card">
      <div class="auth-header">
        ...
        <p>Rejoins la communauté des freelancers camerounais</p>
      </div>
      ...
      <form @submit.prevent="handleRegister" class="auth-form">
        ...
        <div class="auth-footer">
          <p>Déjà inscrit ? <router-link to="/connexion">Connecte-toi ici</router-link></p>
        </div>
      </form>
    </div>
  </div>
  ...
  methods: {
    async handleRegister() {
      ...
      try {
        ...
        const existingUser = users.find(user => user.email === this.form.email)
        if (existingUser) {
        ...
        const newUser = {
          ...
          password: this.form.password, // En production, il faudrait hasher le mot de passe
        ...
        localStorage.setItem('freelancecm_currentUser', JSON.stringify(newUser))
...
.auth-container {
  min-height: 100vh;
  background: linear-gradient(135deg, #007a5e, #1d2d3a);
  display: flex;
  align-items: center;
